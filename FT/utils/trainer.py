...
class Trainer:
    ...
    def compute_loss(self, model, inputs):
        ...
        """
        inputs['input_ids'] = torch.cat((50256*torch.ones((inputs['input_ids'].shape[0], 1), dtype=int).to(inputs['labels'].device), inputs['input_ids']), dim = 1)
        inputs['labels'] = torch.cat((50256*torch.ones((inputs['labels'].shape[0], 1), dtype=int).to(inputs['labels'].device), inputs['labels']), dim = 1)
